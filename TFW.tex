...
\begin{sectionbox}
	...
	\end{itemize}
	\includegraphics[width=\textwidth]{./img/dualität_tabelle.png}
...
\begin{sectionbox}
	...
	$$
	\begin{aligned}
	S(r, \vartheta, \varphi) &=\frac{1}{2 Z_{\mathrm{F}, 0}}|E(\vartheta, \varphi)|^{2} \\
	&=\frac{1}{2 Z_{\mathrm{F}, 0}} \frac{U_{0}^{2}}{16 \pi^{2} r^{2}}\left|C_{\mathrm{t}}(\vartheta, \varphi)\right|^{2}
	\end{aligned}
	$$
	Effektive Wirkfläche:
	$A_{\mathrm{e}}=\frac{\lambda^{2}}{4 \pi} G_{\mathrm{r}}$\\
	Sendeantenne optimal ausgerichtet:\\
	$\left|C_{t}(\vartheta, \varphi)\right|=1 \Rightarrow S(r, \vartheta, \varphi)=S_{\max }(r)$\\
	Empfangsantenne optimal ausgerichtet:\\
	$\left|C_{r}(\vartheta, \varphi)\right|=1 \Rightarrow P_{\mathrm{r}}=P_{\mathrm{r}, \max }$\\
	\includegraphics[width=\textwidth]{./img/sender_empfangs_charakter.png}
	% $S_{i}=\frac{P_{t}}{4 \pi r^{2}}$
	\includegraphics[width=\textwidth]{./img/blockschaltbild_Übertragungsstrecke.png}
	Beispiel: \\
