...
Ansatz: $\nabla \times \mathbf{J} = -j\omega\kappa \mathbf{B}$ und $\nabla\times\mathbf{B} = \mu \mathbf{J}$.\\
...
Feldwellenwiederstand: $Z_F = \frac{\mu}{\epsilon}$, $Z_{F_0} = \frac{\mu_0}{\epsilon_0} \approx 120\pi\Omega \approx 377\Omega$\\
...
\begin{sectionbox}
	...
	\end{emphbox}
Verlustloste Medien: $\omega^2\epsilon\mu$ reell.
\end{sectionbox}
\begin{sectionbox}
...
\begin{sectionbox}
	...
	$\rho = \frac{Z_{F2} - Z_{F1}}{Z_{F1} + Z_{F2}}$ & $\tau = \frac{2Z_{F2}}{Z_{F1} + Z_{F2}}$\\
	\end{tablebox}
	Stehwellenverhältnis: SWR = $\frac{|E_1|_{max}}{|E_1|_{min}} = \frac{1 + |\rho|}{1-|\rho|}$
\end{sectionbox}
...
\begin{sectionbox}
	...
	\begin{itemize}
		...
		\subitem Dielektrische Wand: Leitung mit anderem Wellenwiederstand
...
\begin{sectionbox}
	...
	\begin{emphbox}
		$E(x,y,z) = E_0\frac{w_0}{w(z)}exp\left(-\frac{x^2+y^2}{w(z)^2} -jkz - \frac{j\pi (x^2+y^2)}{\lambda R(z)} +jarctan\frac{\lambda z}{\pi \omega_0^2}\right)$
	\end{emphbox}
	\begin{itemize}
		\item Amplitudenterm: $-\frac{x^2+y^2}{w(z)^2}$
		\item Phasenterm: $-jkz$
		\item Krümmung der Phasenfronten: $- \frac{j\pi (x^2+y^2)}{\lambda R(z)}$
		\item Korrekturfaktor (damits aufgeht): $+jarctan\frac{\lambda z}{\pi \omega_0^2}$
	\end{itemize}
	Für große Werte von $|z|$ schmiegt sich das Hyperboloid asymptotisch einem Kegel an mit dem Öffnungswinkel:\\
	$\theta_0 = arctan\frac{2w_0}{b}$\\
...
\begin{sectionbox}
	...
	\end{tablebox}
Im Lastwiederstand umgesetzte Leistung: $P_L = \frac{|V_+|^2}{Z_0}(1-|\rho|^2)$
\end{sectionbox}
...
Bei höheren Frequenzen werden fast ausschließlich komplexe Amplituden der hineinlaufenden Wellen ($a_1$ bw. $a_2$) und der herauslaufenden Wellen ($b_1$ bzw. $b_2$) als Torgrößen verwendet.
\end{sectionbox}
\begin{sectionbox}
