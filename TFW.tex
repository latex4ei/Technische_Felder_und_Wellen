...
\begin{sectionbox}
	...
	\end{emphbox}
	\begin{center}\includegraphics[width = 6.5cm]{green_zsm.png}\end{center}
\end{sectionbox}

\begin{sectionbox}
	\subsection{Spiegelungsprinzip}
	\begin{center}\includegraphics[width = 6.5cm]{spiegelungsprinzip.png}\end{center}
\end{sectionbox}
...
\begin{sectionbox}
	...
	Ausbreitungskonstante: $\gamma = \alpha + j\beta = jk$\\
	Wellenzahl: $ k = \beta - j \alpha$\\
	...
	$\gamma = j\beta$, $\alpha=0$, $\beta = \sqrt{k^2 - q^2}$\\

	% Ausbreitungseigenschaften:
	\begin{emphbox}
		$\frac{k_z}{k} = \frac{\lambda}{\lambda_G} = \sqrt{1- \left(\frac{f_c}{f}\right)^2}$\\
		$\lambda_{g}=\frac{2 \pi}{\beta}=\frac{2 \pi}{\sqrt{k^{2}-q^{2}}} \geq \lambda$ (g: guided)\\
		$Z_{F H} = \frac{j \omega \mu}{\gamma_{H}}=\frac{\omega \mu}{\beta_{H}}=\frac{Z_{F}}{\sqrt{1 - \left(\frac{f_c}{f}\right)^2}}$\\
		$Z_{F E} = \frac{\gamma_{E}}{j \omega \varepsilon}=\frac{\beta_{E}}{\omega \varepsilon}=\sqrt{1 - \left(\frac{f_c}{f}\right)^2}Z_{F}$\\
	\end{emphbox}
...
\begin{sectionbox}
	...
	$\alpha=\frac{2 \pi}{\lambda_{H E W}} \sqrt{\left(\frac{\lambda_{H E W}}{\lambda_{c}}\right)^{2}-1}=\underbrace{\frac{2 \pi}{\lambda_{c}}}_{\alpha_{c}} \sqrt{1-\left(\frac{f}{f_{c}}\right)^{2}}$\\
...
\begin{array}{ll}	
	...
	\text { Gesendete Leistung (ohne } & P_{t 0} \\
	\text { Verluste) } & P_{t} \\
	\text { Gesendete Leistung } & S(r, \vartheta, \varphi) \\
	\text { Leistungsflussdichte } & S_{i}(r)=\frac{P}{4 \pi r^{2}} \\
	\text { Leistungsflussdichte (isotroper } & \\
	\text { Kugelstrahler) } & C_{t}(\vartheta, \varphi)=\left.\frac{E(\vartheta, \varphi)}{E_{\max }}\right|_{r=\mathrm{konst}} \\
	\text { Strahlungscharakteristik } & D_{t}=\left.\frac{S_{\max }}{S_{i}}\right|_{P_{t}}=\frac{4 \pi r^{2} S_{\max }}{P_{t}} \\
	\text { Direktivität } & G_{t}=\left.\frac{S_{\max }}{S_{i}}\right|_{P_{t 0}}=\frac{4 \pi r^{2} S_{\max }}{P_{t 0}} \\
	\text { Gewinn } & \eta_{t}=\frac{G_{t}}{D_{t}}
\end{array}
$$
\end{sectionbox}
\begin{sectionbox}
\subsection{Empfangen}
$\begin{array}{ll}\text { Empfangene Leistung (ohne } & P_{r 0} \\ \text { Verluste) } & P_{r} \\ \text { Empfangene Leistung } & \\ & \\ \text { Empfangscharakteristik } & C_{r}(\vartheta, \varphi)=\left.\frac{U(\vartheta, \varphi)}{U_{\max }}\right|_{S=\text { konst. }} \\ \text { effektive Wirkfläche (ohne } & A_{0}=\frac{P_{r 0, \max }}{S}=\frac{\lambda^{2}}{4 \pi} D_{r} \\ \text { Verluste) } & A_{e}=\frac{P_{r, \max }}{S}=\frac{\lambda^{2}}{4 \pi} G_{r} \\ \text { effektive Wirkfläche } & \eta_{r}=\frac{A_{e}}{A_{0}} \\ \text { Wirkungsgrad (ohmsche } & \end{array}$
\end{sectionbox}
\begin{sectionbox}
