...
\begin{sectionbox}
	\subsection{Reziprozität, Symmetrie, Verlustfreiheit}
	\begin{emphbox}
...
\begin{sectionbox}
	...
	\begin{emphbox}
		...
		Maximaler Gewinn für beidseitige Wirkleistungsanpassung: $\rho_{G}=\rho_{E}^{*}=S_{11}^{*} ; \rho_{L}=\rho_{A}^{*}=S_{22}^{*}$\\
...
\begin{sectionbox}
	...
	\end{tablebox}	
	Mit Pointing Vektor: $\vec{S}=\frac{1}{2} \vec{E} \times \vec{H}^{*}$\\
